...
def getReplayFile(replay_id):
    ...
    header = "Date,Time,Venue,Teams,Results,Outcome,Team Points,Team Goals,Team Assists,Team Saves,Team Shots,Team Demos,Team Demos Taken," \
    ...
    cw.write(header + "\n")
    blue_first = str(replay_json["date"].split("T")[0]) + "," + str(replay_json["date"].split("T")[1]) + ",SGL Twitch," +\
...
def getReplayIDs(uploader, replay_date_before, replay_date_after):
    ...
    url = 'https://ballchasing.com/api/replays?uploader=' + uploader + '&replay-date-before=2020-' + \
          ...
          replay_date_after[0] + 'T' + replay_date_after[1] + ':00-06:00&playlist=private&count=1'
    auth = {'Authorization': 'vA9akDv9RQ4TlOGMREPJpbXmFsvOKxkWRr3SYFXM'}
    r = requests.get(url, headers=auth)
...
def main():
    ...
    replays = getReplayIDs("76561198156218146", replay_date_before, replay_date_after)
    # data = apiCallByReplayId('1ef2f98e-d7d9-45a3-b2dd-7e92a2da73b0')
    # print(replays)
